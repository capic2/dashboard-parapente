...
export const useCurrentConditions = (
  siteId?: string
): UseQueryResult<WeatherConditions | WeatherConditions[], Error> => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!validation.success) {
        console.error(
          '❌ Current weather validation failed:',
          validation.error
        );
...
export const useWeatherSources = (): UseQueryResult<WeatherSource[], Error> => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Validate API response with Zod
      const validation = ApiResponseSchema(
        WeatherSourceSchema.array()
      ).safeParse(data);
      if (!validation.success) {
        console.error(
          '❌ Weather sources validation failed:',
          validation.error
        );
...
export const useWeatherHistory = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Validate API response with Zod
      const validation = ApiResponseSchema(
        WeatherConditionsSchema.array()
      ).safeParse(data);
      if (!validation.success) {
        console.error(
          '❌ Weather history validation failed:',
          validation.error
        );
...
export const useWeatherBySource = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const data = await api
        .get(`weather/forecast/${siteId}?source=${sourceId}`)
        .json();

      // Validate API response with Zod
      const validation = ApiResponseSchema(ForecastDaySchema.array()).safeParse(
        data
      );
      if (!validation.success) {
        console.error(
          '❌ Weather by source validation failed:',
          validation.error
        );
        throw new Error(
          `Invalid weather by source: ${validation.error.message}`
        );
      }
...
export const createWeatherQueryFn =
  (siteId: string, dayIndex: number) => async () => {
    ...
    if (!transformedValidation.success) {
      console.error(
        '❌ Transformed weather validation failed:',
