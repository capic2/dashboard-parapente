...
const BuggyComponent = ({ shouldThrow = false }: { shouldThrow?: boolean }) => {
  ...
  return <div className="p-4 bg-green-50 text-green-800 rounded">Component rendered successfully ✓</div>
}
...
WithError.test('should display error UI when error occurs', async ({ canvasElement }) => {
  const canvas = within(canvasElement)
  
  // Verify error message is displayed
  await expect(canvas.getByText(/Une erreur est survenue/i)).toBeInTheDocument()
  ...
  render: () => (
    <ErrorBoundary>
      <div className="p-8 bg-white rounded-xl shadow-md">
        <h2 className="text-xl font-bold text-gray-900 mb-4">Everything is working fine!</h2>
        <p className="text-gray-600">
          The ErrorBoundary is active but no error has occurred, so the children render normally.
        </p>
      </div>
    </ErrorBoundary>
  ),
    ...
    <ErrorBoundary
      fallback={
        <div className="p-8 bg-red-50 border-2 border-red-200 rounded-xl">
          <h2 className="text-xl font-bold text-red-600 mb-2">Custom Error Message</h2>
          <p className="text-red-800">This is a custom fallback UI provided as a prop.</p>
        </div>
      }
    >
      <BuggyComponent shouldThrow={true} />
  ...
  render: () => {
    ...
    return (
      <ErrorBoundary key={shouldError ? 'error' : 'ok'}>
        <div className="p-8 bg-white rounded-xl shadow-md">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Interactive Error Test</h2>
...
export const WithErrorDetails = meta.story({
  ...
  parameters: {
    docs: {
      description: {
        story: 'Shows the error boundary with technical details available in the expandable section.',
      },
    },
  },
})
...
export const NestedBoundaries = meta.story({
  ...
  render: () => (
    <ErrorBoundary
      fallback={<div className="p-4 bg-red-100 text-red-800">Outer boundary caught an error</div>}
    >
      <div className="p-8 bg-gray-50 rounded-xl">
        <h2 className="text-xl font-bold mb-4">Outer Boundary</h2>
        <ErrorBoundary
          fallback={<div className="p-4 bg-yellow-100 text-yellow-800">Inner boundary caught an error</div>}
        >
