...
SITE_NAMES = {"arguel": "Arguel", "mont-poupet": "Mont Poupet", "la-cote": "La Côte"}
...
def parse_gpx(gpx_path, flight_date, flight_time):
    ...
    for trkpt in root.findall(".//gpx:trkpt", GPX_NS):
        ...
        if ele_elem is not None and time_elem is not None:
            ...
            points.append({"lat": lat, "lon": lon, "ele": float(ele_elem.text), "time": point_time})
...
def main():
    ...
    try:
        ...
        for flight in FLIGHTS:
            ...
            print(
                f"✅ {name} - {metrics['duration_minutes']}min, {metrics['distance_km']}km, +{metrics['elevation_gain_m']}m"
            )
