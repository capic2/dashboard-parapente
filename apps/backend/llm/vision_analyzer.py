...
async def analyze_skewt_with_llm(
    ...
    try:
        ...
        if "alertes_securite" in analysis_data and isinstance(
            analysis_data["alertes_securite"], list
        ):
            analysis_data["alertes_securite"] = json.dumps(
                analysis_data["alertes_securite"], ensure_ascii=False
            )
...
async def analyze_emagram_with_fallback(
    ...
    llm_result = await analyze_skewt_with_llm(
        image_path=image_path, station_name=station_name, provider=provider, api_key=api_key
    )
    ...
    stability_result = calculate_stability_indices(
        pressure_hpa=pressure, temperature_c=temperature, dewpoint_c=dewpoint, height_m=height
    )
    ...
    return {
        ...
        "heure_debut_thermiques": (
            hours_result.get("heure_debut_thermiques") if hours_result.get("success") else None
        ),
        "heure_fin_thermiques": (
            hours_result.get("heure_fin_thermiques") if hours_result.get("success") else None
        ),
        "heures_volables_total": (
            hours_result.get("heures_volables_total") if hours_result.get("success") else None
        ),
        ...
        "alertes_securite": json.dumps(
            ["Prévisions basées sur calculs automatiques - vérifier conditions réelles"],
            ensure_ascii=False,
        ),
        # Classic indices
