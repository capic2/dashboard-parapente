...
async def fetch_all_emagrammes_for_spot(
    ...
    from scrapers.meteo_parapente_emagram import fetch_meteo_parapente_emagram, fetch_topmeteo_emagram
    ...
    tasks.append(("meteo-parapente", fetch_meteo_parapente_emagram(
        spot_latitude, spot_longitude, spot_name, output_dir
    )))
    
    # 2. Open-Meteo (generate emagram + LLM analysis)
    tasks.append(("open-meteo", fetch_and_generate_openmeteo(
        spot_latitude, spot_longitude, spot_name, forecast_hour, output_dir
    )))
    
    # 3. TopMeteo (screenshot)
    tasks.append(("topmeteo", fetch_topmeteo_emagram(
        spot_latitude, spot_longitude, spot_name, output_dir
    )))
...
async def fetch_and_generate_openmeteo(
    ...
    sounding = await fetch_sounding_for_spot(
        latitude, longitude, spot_name, forecast_hour
    )
