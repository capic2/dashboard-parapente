...
async def explore_meteo_parapente(
    ...
    try:
        async with async_playwright() as p:
            ...
            if debug_mode:
                logger.info("⚠️  Mode DEBUG: Le navigateur va s'ouvrir. Vous pouvez inspecter la page.")
...
async def explore_and_extract(page, site_name: str, elevation_m: int, days: int, debug: bool):
    ...
    try:
        ...
        # ============================================================
        logger.info(f"🔎 Recherche du champ de recherche...")
...
async def extract_day_forecast(page, elevation_m: int, selectors: dict, debug: bool):
    ...
    try:
        ...
        for selector, expected_tag in altitude_strategies:
            try:
                elements = await forecast_container.locator(selector).all() if forecast_container else await page.locator(selector).all()
                if elements:
...
async def main():
    ...
    print(f"Jours:     {days}")
    print(f"Debug:     OUI (navigateur visible)")
    print("="*60 + "\n")
