...
from strava import download_gpx, get_activity_details, parse_gpx
...
async def strava_webhook_verification(
    request: Request, hub_mode: str = None, hub_verify_token: str = None, hub_challenge: str = None
):
    """
...
@router.post("/strava")
async def strava_webhook_handler(request: Request, db: Session = Depends(get_db)):
    """
...
async def process_strava_activity(activity_id: str, db: Session = None):
    ...
    try:
        ...
        if activity_type not in ["Flight", "Workout", "Ride"]:
            logger.info(
                f"⚠️ Activity {activity_id} is not a flight (type={activity_type}), ignoring"
            )
            return
...
def match_site_by_name(db: Session, location: str, location_city: str) -> str | None:
    """
    ...
    site = db.query(Site).filter(Site.name.ilike(f"%{location}%")).first()
    ...
    # Try match on city
    if location_city:
        site = db.query(Site).filter(Site.name.ilike(f"%{location_city}%")).first()
...
def match_site_by_coordinates(
    db: Session, lat: float, lon: float, threshold_km: float = 5.0
) -> str | None:
    """
    ...
    if closest_site and min_distance <= threshold_km:
        logger.info(
            f"✅ Matched site by coordinates: {closest_site.name} ({min_distance:.2f}km away)"
        )
