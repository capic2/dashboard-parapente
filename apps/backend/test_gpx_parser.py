...
def parse_gpx_file_from_string(gpx_content: str):
    ...
    if coordinates:
        print(
            f"🔍 DEBUG First coord - ele:{coordinates[0]['elevation']}m, ts:{coordinates[0]['timestamp']}"
        )
...
with open("db/gpx/strava_16394749726.gpx") as f:
    content = f.read()
...
if coords:
    ...
    for i in range(min(5, len(coords))):
        c = coords[i]
        print(
            f'  {i}: lat={c["lat"]:.6f}, lon={c["lon"]:.6f}, ele={c["elevation"]}m, ts={c["timestamp"]}'
        )
