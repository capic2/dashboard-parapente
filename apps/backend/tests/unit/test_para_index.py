...
class TestCalculateParaIndex:
    ...
    def test_excellent_conditions(self):
        ...
        assert any(
            word in result["explanation"].lower() for word in ["optimal", "favorable", "modéré"]
        )
    ...
    def test_poor_conditions_strong_wind(self):
        ...
        assert (
            "dangereux" in result["explanation"].lower() or "fort" in result["explanation"].lower()
        )
    ...
    def test_dangerous_gusts(self):
        ...
        assert result["para_index"] <= 50
        assert (
            "rafales" in result["explanation"].lower()
            or "dangereux" in result["explanation"].lower()
        )
...
class TestFormatSlotsSummary:
    ...
    def test_format_single_slot(self):
        ...
        slots = [{"start_hour": 12, "end_hour": 14, "verdict": "🟢", "reasons": []}]
