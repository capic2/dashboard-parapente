...
def test_overlay_inputs_prefer_the_flight_gpx_from_database(tmp_path: Path) -> None:
    ...
    for path in (zepp, configured_gpx, first_flight, latest_flight, generated_video):
...
def test_overlay_inputs_do_not_replace_a_missing_database_gpx(tmp_path: Path) -> None:
    zepp = tmp_path / "Zepp-flight.gpx"
    zepp.write_bytes(b"gpx")
    ...
    assert gpx_path == tmp_path / "missing.gpx"
