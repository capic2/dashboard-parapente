...
async def download_gpx(activity_id: str) -> str | None:
    """
...
def parse_gpx(gpx_content: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not elevations:
            return {"success": False, "error": "No elevation data in GPX"}
...
async def get_activity_details(activity_id: str) -> dict[str, Any] | None:
    """
...
async def get_activities_by_period(
    date_from: str, date_to: str, activity_type: str = "Workout", per_page: int = 50
) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            while True:
                ...
                filtered = [
                    ...
                    if act.get("type") == activity_type or act.get("sport_type") == activity_type
                ]

                logger.debug(
                    f"Page {page}: {len(filtered)} activities match type '{activity_type}'"
                )
        ...
        logger.info(
            f"✅ Retrieved {len(all_activities)} '{activity_type}' activities from {date_from} to {date_to}"
        )
...
def match_site_by_coordinates(lat: float, lon: float, sites: list[dict[str, Any]]) -> str | None:
    """
