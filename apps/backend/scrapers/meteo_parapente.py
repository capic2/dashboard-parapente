        ...
        async def _get_latest_run(self) -> str:
            ...
            try:
                async with self._get_client() as client:
                    ...
                    logger.info(f"Using model run: {run_str} (updated: {latest_run.get('update', 'unknown')})")
                    return run_str
        ...
        async def _build_params(self, lat: float, lon: float, **kwargs) -> Dict[str, Any]:
            ...
            if site_name:
                ...
                if precise_lat and precise_lon:
                    logger.info(f"Using precise coordinates for {site_name}: {precise_lat:.4f}, {precise_lon:.4f}")
                    final_lat, final_lon = precise_lat, precise_lon
                else:
                    logger.warning(f"Could not find precise coordinates for {site_name}, using provided: {lat:.4f}, {lon:.4f}")
        ...
        def extract_hourly_forecast(
            ...
            for hour_str, hour_data in sorted(hourly_data_dict.items()):
                try:
                    ...
                    temps = hour_data.get("tc", [])  # Temperature (°C) - using "tc" from sounding plot
                    ...
                    u = u_components[closest_idx] if closest_idx < len(u_components) else u_components[0]
                    v = v_components[closest_idx] if closest_idx < len(v_components) else v_components[0]
            ...
            logger.info(f"Extracted {len(hourly_forecasts)} hourly forecasts from meteo-parapente API")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=15) as client:
                response = await client.get(
                    "https://data0.meteo-parapente.com/data.php",
                    params=params
                )
                response.raise_for_status()
