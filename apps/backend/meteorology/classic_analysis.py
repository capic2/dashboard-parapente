...
def calculate_stability_indices(
    ...
    try:
        ...
        for i in range(len(pressure_hpa)):
            if (
                pressure_hpa[i] is not None
                and temperature_c[i] is not None
                and dewpoint_c[i] is not None
            ):
        ...
        if height_m:
            h = (
                np.array(
                    [height_m[i] if height_m[i] is not None else np.nan for i in valid_indices]
                )
                * units.meter
            )
        else:
...
def calculate_wind_shear(
    pressure_hpa: list[float], height_m: list[float], wind_u_ms: list[float], wind_v_ms: list[float]
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {"success": False, "error": f"Wind shear calculation failed: {str(e)}"}


def estimate_flyable_hours(
    cape_jkg: float, plafond_m: int | None, stabilite: str, latitude: float
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {"success": False, "error": f"Flyable hours estimation failed: {str(e)}"}
