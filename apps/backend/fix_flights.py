...
def main():
    ...
    # Get all flights
    cursor.execute("SELECT id, site_id, flight_date FROM flights WHERE title IS NULL OR max_altitude_m IS NULL")
    ...
    for flight_id, site_id, flight_date in flights:
        ...
        if not parsed:
            print(f"  ❌ Could not parse flight ID")
            continue
        ...
        if gpx_file:
            ...
            if max_altitude:
                ...
                print(f"  ⚠️  Could not extract altitude from GPX")
        else:
            print(f"  ⚠️  No matching GPX file found")
        ...
        if max_altitude:
            ...
            cursor.execute(
                "UPDATE flights SET title = ? WHERE id = ?",
                (title, flight_id)
            )
