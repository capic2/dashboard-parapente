...
async def fetch_from_enabled_sources(
    ...
    try:
        ...
        if not enabled_sources:
            ...
            fallback = (
                db.query(WeatherSourceConfig)
                .filter(WeatherSourceConfig.source_name == "open-meteo")
                .first()
            )
            if fallback:
        ...
        logger.info(
            f"Fetching weather from {len(enabled_sources)} enabled sources: "
...
async def aggregate_forecasts(
    ...
    if sources is not None:
        logger.warning(
            "Parameter 'sources' is deprecated and ignored. Using enabled sources from database instead."
        )

    # Delegate to new instrumented function
    return await fetch_from_enabled_sources(
        lat=lat, lon=lon, day_index=day_index, site_name=site_name, elevation_m=elevation_m, db=db
    )
...
def calculate_consensus(normalized: dict[str, Any]) -> dict[str, Any]:
    ...
    for hour_data in normalized.get("normalized", []):
        ...
        wind_direction_raw = (
            hour_data["wind_direction"].copy() if hour_data["wind_direction"] else []
        )
        ...
        for i, source in enumerate(sources_list):
            if i < len(hour_data["temperature"]):
                sources_data[source]["temperature"] = (
                    round(hour_data["temperature"][i], 1)
                    if hour_data["temperature"][i] is not None
                    else None
                )
            if i < len(hour_data["wind_speed"]):
                sources_data[source]["wind_speed"] = (
                    round(hour_data["wind_speed"][i], 1)
                    if hour_data["wind_speed"][i] is not None
                    else None
                )
            if i < len(hour_data["wind_gust"]):
                sources_data[source]["wind_gust"] = (
                    round(hour_data["wind_gust"][i], 1)
                    if hour_data["wind_gust"][i] is not None
                    else None
                )
            if i < len(hour_data["wind_direction"]):
                sources_data[source]["wind_direction"] = (
                    round(hour_data["wind_direction"][i], 1)
                    if hour_data["wind_direction"][i] is not None
                    else None
                )
            if i < len(hour_data["precipitation"]):
                sources_data[source]["precipitation"] = (
                    round(hour_data["precipitation"][i], 2)
                    if hour_data["precipitation"][i] is not None
                    else None
                )
            if i < len(hour_data["cloud_cover"]):
                sources_data[source]["cloud_cover"] = (
                    round(hour_data["cloud_cover"][i], 1)
                    if hour_data["cloud_cover"][i] is not None
                    else None
                )
            if i < len(hour_data["cape"]):
                sources_data[source]["cape"] = (
                    round(hour_data["cape"][i], 1) if hour_data["cape"][i] is not None else None
                )
            if i < len(hour_data["lifted_index"]):
                sources_data[source]["lifted_index"] = (
                    round(hour_data["lifted_index"][i], 1)
                    if hour_data["lifted_index"][i] is not None
                    else None
                )
        ...
        consensus.append(
            {
                ...
                "temperature": round(temp_avg, 1) if temp_avg is not None else None,
                "temperature_confidence": round(temp_conf, 2),
                "wind_speed": round(wind_avg, 1) if wind_avg is not None else None,
                "wind_confidence": round(wind_conf, 2),
                "wind_gust": round(gust_avg, 1) if gust_avg is not None else None,
                "gust_confidence": round(gust_conf, 2),
                "wind_direction": round(dir_avg, 1) if dir_avg is not None else None,
                "direction_confidence": round(dir_conf, 2),
                "precipitation": round(precip_avg, 2) if precip_avg is not None else None,
                "precipitation_confidence": round(precip_conf, 2),
                "cloud_cover": round(cloud_avg, 1) if cloud_avg is not None else None,
                "cloud_confidence": round(cloud_conf, 2),
                "cape": round(cape_avg, 1) if cape_avg is not None else None,
...
def extract_sunrise_sunset(aggregated: dict[str, Any], day_index: int) -> tuple:
    ...
    logger.debug(
        f"extract_sunrise_sunset: day_index={day_index}, "
        f"sources={list(aggregated.get('sources', {}).keys())}, "
        f"open_meteo success={open_meteo_data.get('success')}"
    )

    if not open_meteo_data.get("success"):
        logger.debug("No successful open-meteo data for sunrise/sunset extraction")
        return None, None
...
async def get_normalized_forecast(
    ...
    logger.debug(f"get_normalized_forecast: sunrise={sunrise}, sunset={sunset}")
...
def calculate_daily_para_index(
    wind_avg: float, precip_total: float, min_wind: float, max_wind: float
) -> int:
    """
