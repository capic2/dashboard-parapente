...
export const useUpdateWeatherSource = () => {
  ...
  return useMutation({
    mutationFn: async ({ sourceName, data }: { sourceName: string; data: WeatherSourceUpdate }) => {
      return await api.patch(`weather-sources/${sourceName}`, { json: data }).json<WeatherSource>()
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['weather-sources'] })
      queryClient.invalidateQueries({ queryKey: ['weather-sources', variables.sourceName] })
    },
  })
}
...
export const useDeleteWeatherSource = () => {
  ...
  return useMutation({
    mutationFn: async (sourceName: string) => {
      return await api.delete(`weather-sources/${sourceName}`).json<{ success: boolean; message: string }>()
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['weather-sources'] })
    },
...
export const useTestWeatherSource = () => {
  return useMutation({
    mutationFn: async ({ sourceName, lat, lon }: { sourceName: string; lat: number; lon: number }) => {
      return await api
