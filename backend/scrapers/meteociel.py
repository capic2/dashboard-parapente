...
class MeteocielScraper(BaseScraper):
    ...
    async def _get_insee_code(self, city_name: str) -> Optional[str]:
        """
    ...
    async def fetch(self, lat: float, lon: float, **kwargs) -> Dict[str, Any]:
        ...
        if not site_name:
            return self._build_response(
                success=False,
                error="site_name is required for meteociel scraper"
            )
        ...
        try:
            ...
            city_normalized = ''.join(
                c for c in unicodedata.normalize('NFD', site_name)
    ...
    def _parse_forecast_tables(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for table in tables:
            ...
            logger.info(f"Found forecast table")
        ...
        if len(hourly_data) < 20:
            logger.warning(f"Only extracted {len(hourly_data)} hours from Meteociel, expected ~40-50")
