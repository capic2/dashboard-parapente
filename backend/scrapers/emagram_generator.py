...
def generate_skewt_image(
    ...
    try:
        ...
        if len(valid_indices) < 5:
            return {
                "success": False,
                "error": "Insufficient valid data points (need at least 5)"
            }
        ...
        if show_parcel and len(valid_indices) >= 10:
            try:
                ...
                parcel_prof = mpcalc.parcel_profile(
                    p_valid,
                    T_valid[surface_idx],
                    Td_valid[surface_idx]
                )
                
                # Plot parcel path
                skew.plot(p_valid, parcel_prof, 'k', linewidth=2, linestyle='--', 
                         label='Parcel Path', alpha=0.7)
        ...
        return {
            "success": False,
            "error": f"Failed to generate Skew-T diagram: {str(e)}"
        }


def generate_emagram_from_wyoming(
    wyoming_result: Dict[str, Any],
    ...
    if not wyoming_result.get("success"):
        return {
            "success": False,
            "error": "Wyoming data fetch was not successful"
        }
    ...
    if not data:
        return {
            "success": False,
            "error": "No sounding data in Wyoming result"
        }
