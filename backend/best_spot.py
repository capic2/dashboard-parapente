...
def get_cache_module():
    ...
    try:
        from cache import get_redis, CACHE_TTL
...
def parse_wind_direction(wind_dir: str) -> Optional[float]:
    ...
    try:
        ...
        for site in sites:
            try:
                ...
                if midday_hours:
                    avg_wind_speed = sum(h.get("wind_speed", 0) for h in midday_hours) / len(midday_hours)
        ...
        if best_site['score'] < 20:
            best_site['reason'] = f"Conditions défavorables partout (meilleur: {best_site['site']['name']}, score {int(best_site['score'])})"
        
        logger.info(f"✅ Best spot: {best_site['site']['name']} (score: {best_site['score']:.1f}, Para-Index: {best_site['paraIndex']})")
