...
async def debug_weather():
    ...
    try:
        ...
        if result.get('sources'):
            ...
            for source_name, source_data in result.get('sources', {}).items():
                if isinstance(source_data, dict):
                    ...
                    print(f"  - {source_name:20} Success: {success:5}  Hours: {hourly:3}  Error: {error[:50]}")
        ...
        if daily:
            print(f"✅ SUCCESS! Got daily data:")
            ...
            print(f"❌ FAILED! get_daily_aggregate returned None")
            ...
            if sunrise and sunset and consensus:
                try:
                    ...
                    flyable = [h for h in consensus if sunrise_hour <= h.get('hour', 0) <= sunset_hour]
                    print(f"   ✅ Flyable hours ({sunrise_hour}h-{sunset_hour}h): {len(flyable)} hours")
                    
                    if not flyable:
                        print("   ⚠️  No flyable hours!")
                    else:
                        # Check temps and winds
                        temps = [h.get("temperature") for h in flyable if h.get("temperature") is not None]
                        winds = [h.get("wind_speed") for h in flyable if h.get("wind_speed") is not None]
