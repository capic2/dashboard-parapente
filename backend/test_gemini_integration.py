...
def test_gemini_analysis(api_key, screenshot_paths):
    ...
    try:
        ...
        if not isinstance(result['plafond_thermique_m'], int):
            logger.warning(f"⚠️  plafond_thermique_m should be int, got {type(result['plafond_thermique_m'])}")
        
        if not isinstance(result['force_thermique_ms'], (int, float)):
            logger.warning(f"⚠️  force_thermique_ms should be float, got {type(result['force_thermique_ms'])}")
        
        if not isinstance(result['score_volabilite'], int):
            logger.warning(f"⚠️  score_volabilite should be int, got {type(result['score_volabilite'])}")
        
        if not isinstance(result['alertes_securite'], list):
            logger.warning(f"⚠️  alertes_securite should be list, got {type(result['alertes_securite'])}")
