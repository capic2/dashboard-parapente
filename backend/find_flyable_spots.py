    ...
    if slots and isinstance(slots, list) and len(slots) > 0:
        ...
        for slot in slots:
            if isinstance(slot, dict) and 'start' in slot:
                ...
                # Alternative format
                print(f"  • {slot.get('time', '?')} (Para-Index: {slot.get('para_index', '?')}/100)")
    else:
...
def find_flyable_spots(
    ...
    try:
        ...
        spots_sorted = sorted(
            spots,
            key=lambda x: x.get('weather', {}).get('para_index', -1),
            reverse=True
        )
        ...
        if best_spot:
            ...
            print(f"   curl \"http://localhost:8001/api/spots/weather/{best_spot['id']}?days=3\" | python -m json.tool")
...
def main():
    ...
    if "--gps" in args:
        try:
            ...
            find_flyable_spots(lat=lat, lon=lon, radius_km=radius_km, spot_type=spot_type, limit=limit)
        except (IndexError, ValueError):
