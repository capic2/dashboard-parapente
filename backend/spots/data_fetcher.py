...
# Data source URLs
OPENAIP_FRANCE_URL = "https://storage.googleapis.com/29f98e10-a489-4c82-ae5e-489dbcd4912f/fr_hgl.json"
    ...
    try:
        ...
        for item in data:
            try:
                ...
                spot_id = f"openaip_{openaip_id}" if openaip_id else f"openaip_{hashlib.md5(item.get('name', '').encode()).hexdigest()}"
...
def merge_duplicate_spots(
    openaip_spots: List[Dict],
    pgs_spots: List[Dict]
) -> List[Dict]:
    """
    ...
    logger.info(f"✓ Merge complete: {stats['merged']} merged, {stats['openaip_only']} OA-only, {stats['pgs_only']} PGS-only → {stats['total']} total")
...
def sync_to_database(db: Session) -> Dict[str, int]:
    ...
    for spot_data in all_spots:
        try:
            ...
            existing = db.query(ParaglidingSpot).filter(ParaglidingSpot.id == spot_data['id']).first()
