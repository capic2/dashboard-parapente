...
def link_sites_to_spots(db: Session) -> Dict:
    ...
    for site in sites:
        ...
        candidates = db.query(ParaglidingSpot).filter(
            ParaglidingSpot.latitude.between(
                site.latitude - 0.01,  # ~1km buffer
                site.latitude + 0.01
            ),
            ParaglidingSpot.longitude.between(
                site.longitude - 0.01,
                site.longitude + 0.01
            )
        ).all()
        ...
        if closest_spot and closest_distance <= MATCH_DISTANCE_THRESHOLD_KM:
            ...
            if (abs(site.latitude - closest_spot.latitude) > 0.0001 or
                abs(site.longitude - closest_spot.longitude) > 0.0001):
            ...
            logger.warning(
                f"No close match for {site.name} "
                f"(closest: {closest_distance:.2f}km away)"
            )
            stats["not_found"] += 1
