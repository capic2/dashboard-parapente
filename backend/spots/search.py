...
def get_spot_by_id(db: Session, spot_id: str) -> Optional[Dict]:
    """
...
def get_sync_status(db: Session) -> Dict:
    ...
    # Count by source
    openaip_count = db.query(func.count(ParaglidingSpot.id)).filter(
        ParaglidingSpot.source == "openaip"
    ).scalar()
    
    pgs_count = db.query(func.count(ParaglidingSpot.id)).filter(
        ParaglidingSpot.source == "paraglidingspots"
    ).scalar()
    
    merged_count = db.query(func.count(ParaglidingSpot.id)).filter(
        ParaglidingSpot.source == "merged"
    ).scalar()
    
    # Count by type
    takeoff_count = db.query(func.count(ParaglidingSpot.id)).filter(
        ParaglidingSpot.type == "takeoff"
    ).scalar()
    
    landing_count = db.query(func.count(ParaglidingSpot.id)).filter(
        ParaglidingSpot.type == "landing"
    ).scalar()
    
    # Get last sync time
    last_synced_spot = db.query(ParaglidingSpot).order_by(
        ParaglidingSpot.last_synced.desc()
    ).first()
